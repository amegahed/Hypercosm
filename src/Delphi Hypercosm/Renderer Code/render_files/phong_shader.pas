...
{***************************************************************}
{ |\  /|                                               We Put   }
{ | >< Hypercosm            phong_shader               3d       }
{ |/  \|                                               To Work! }
{***************************************************************}
{                                                               }
{       This module draws a phong shaded rendering of           }
...
interface
uses
  trans_stack, state_vars, b_rep, z_renderer;


{*************************************************}
{ routines for Phong shading and drawing surfaces }
{*************************************************}
procedure Phong_shade_surface(z_renderer: z_renderer_type;
...
uses
  SysUtils,
  drawable, colors, vectors, trans, coord_axes, eye, project, geometry, topology,
  object_attr, coord_stack, xform_b_rep, z_buffer, z_polygons, z_clip,
  z_triangles, z_pipeline, clip_planes, hidden_lines, render_lines, coords,
  shade_b_rep, meshes, z_phong_polygons, z_phong_lines;
...
procedure Fill_faces(z_renderer: z_renderer_type;
  face_ptr: face_ptr_type);
var
            ...
            begin
              ...
              while (directed_edge_ptr <> nil) do
                begin
                  ...
                  z_renderer.Add_vertex(point_data_ptr^.trans_point);
                  directed_edge_ptr := directed_edge_ptr^.next;
                end;
              cycle_ptr := cycle_ptr^.next;
              if (cycle_ptr <> nil) then
                z_renderer.Begin_hole;
            end;
          Phong_shade_z_polygon(z_renderer);
        end;
      face_ptr := face_ptr^.next;
    end;
...
procedure Phong_shade_faces(z_renderer: z_renderer_type;
  face_ptr: face_ptr_type;
  shading: surface_shading_type);
var
  vertex_ptr: vertex_ptr_type;
...
end; {procedure Phong_shade_faces}


procedure Phong_shade_edges(z_renderer: z_renderer_type;
...
begin
  while (edge_ptr <> nil) do
    begin
      ...
      Phong_shade_z_line(z_renderer);
      edge_ptr := edge_ptr^.next;
    end;
end; {procedure Phong_shade_edges}


procedure Phong_shade_vertices(z_renderer: z_renderer_type;
  vertex_ptr: vertex_ptr_type);
var
  point_data_ptr: point_data_ptr_type;
begin
  z_renderer.Begin_points;
  while (vertex_ptr <> nil) do
    begin
      point_data_ptr := Get_point_data(vertex_ptr^.point_ptr^.index);
      z_renderer.Add_vertex(point_data_ptr^.trans_point);
  ...
  z_renderer.Draw_points;
end; {procedure Phong_shade_vertices}


procedure Phong_shade_b_rep(z_renderer: z_renderer_type;
  surface_ptr: surface_ptr_type);
var
...
begin
  ...
  if face_ptr <> nil then
    begin
      ...
      {*************************************************}
      if Supports(z_renderer.drawable, ditherable_type, ditherable) then
        ditherable.Set_dither_mode(false);
      Set_z_color(background_color);
      Fill_faces(z_renderer, face_ptr);

      {*********************************************}
      { next draw the polygon into z buffer shading }
      { only the front facing (non hidden) surface. }
      {*********************************************}
      if ditherable <> nil then
        ditherable.Set_dither_mode(true);
      Phong_shade_faces(z_renderer, face_ptr, surface_ptr^.shading);
    end
  else
    begin
      {************}
      ...
      if (edge_ptr <> nil) then
        Phong_shade_edges(z_renderer, edge_ptr)
      else
        begin
          {***************}
          { draw vertices }
          ...
          if (vertex_ptr <> nil) then
            Phong_shade_vertices(z_renderer, vertex_ptr);
        end;
    end;
end; {procedure Phong_shade_b_rep}
...
procedure Fill_visible_faces(z_renderer: z_renderer_type;
  face_ptr: face_ptr_type);
var
  cycle_ptr: cycle_ptr_type;
  directed_edge_ptr: directed_edge_ptr_type;
...
procedure Phong_z_visible_vertex(z_renderer: z_renderer_type;
  vertex_ptr: vertex_ptr_type;
  shading: surface_shading_type;
  face_normal: vector_type);
var
...
procedure Phong_shade_visible_faces(z_renderer: z_renderer_type;
  face_ptr: face_ptr_type;
  shading: surface_shading_type);
var
  vertex_ptr: vertex_ptr_type;
            ...
            begin
              ...
              while (directed_edge_ptr <> nil) do
                begin
                  ...
                  Phong_z_visible_vertex(z_renderer, vertex_ptr, shading, face_normal);
                  directed_edge_ptr := directed_edge_ptr^.next;
                end;
              cycle_ptr := cycle_ptr^.next;
            end;
...
end; {procedure Phong_shade_visible_faces}


procedure Phong_shade_visible_edges(z_renderer: z_renderer_type;
  edge_ptr: edge_ptr_type);
begin
  while (edge_ptr <> nil) do
    begin
      Begin_z_line;
      Phong_z_visible_vertex(z_renderer, edge_ptr^.vertex_ptr1, smooth_shading, z_vector);
      Phong_z_visible_vertex(z_renderer, edge_ptr^.vertex_ptr2, smooth_shading, z_vector);
      Phong_shade_z_line(z_renderer);
      edge_ptr := edge_ptr^.next;
    end;
end; {procedure Phong_shade_visible_edges}


procedure Phong_shade_visible_vertices(z_renderer: z_renderer_type;
  vertex_ptr: vertex_ptr_type);
begin
  Begin_z_points;
  while (vertex_ptr <> nil) do
  ...
  z_renderer.Draw_points;
end; {procedure Phong_shade_visible_vertices}


procedure Phong_shade_visible_b_rep(z_renderer: z_renderer_type;
  surface_ptr: surface_ptr_type);
var
  ditherable: ditherable_type;
  face_ptr: face_ptr_type;
  ...
  if face_ptr <> nil then
    begin
      ...
      if Supports(z_renderer.drawable, ditherable_type, ditherable) then
        ditherable.Set_dither_mode(false);
      Set_z_color(background_color);
      Fill_visible_faces(z_renderer, face_ptr);
      ...
      if ditherable <> nil then
        ditherable.Set_dither_mode(true);
      Phong_shade_visible_faces(z_renderer, face_ptr, surface_ptr^.shading);
    end
  else
    begin
      {************}
      ...
      edge_ptr := surface_ptr^.topology_ptr^.edge_ptr;
      if (edge_ptr <> nil) then
        Phong_shade_visible_edges(z_renderer, edge_ptr)
      else
        begin
          {***************}
          { draw vertices }
          ...
          vertex_ptr := surface_ptr^.topology_ptr^.vertex_ptr;
          if (vertex_ptr <> nil) then
            Phong_shade_visible_vertices(z_renderer, vertex_ptr);
        end;
    end;
end; {procedure Phong_shade_visible_b_rep}
...
begin
  ...
  {***********************************}
  if clipping then
    Phong_shade_b_rep(z_renderer, surface_ptr)
  else
    Phong_shade_visible_b_rep(z_renderer, surface_ptr)
end; {procedure Phong_shade_surface}
